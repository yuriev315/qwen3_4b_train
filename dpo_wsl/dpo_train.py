...
from trl.trainer.utils import entropy_from_logits, get_kbit_device_map, selective_log_softmax
...
# LR 1e-5 + 1 epoch: enough signal without drifting far from the SFT checkpoint.
# beta 0.25: with sigmoid_norm, logged loss stays near ln(2)≈0.6931 until margins grow;
#   track rewards/margins and dpo/delta. Override with DPO_BETA / DPO_LR.
...
# OOM mitigation: precompute_ref_log_probs (+ optional activation_offloading on native Linux).
# activation_offloading is auto-disabled on WSL — causes intermittent "CUDA device not ready" on backward.
...
HYPERPARAMS = {
    ...
    "use_weighting": False,  # WPO; disabled when use_sample_weight=True unless DPO_USE_WEIGHTING=1
    "use_sample_weight": True,  # JSONL weight column → weighted mean in DPO loss
...
@dataclass
class SampleWeightPreferenceCollator(DataCollatorForPreference):
    """Preference collator that also batches the JSONL `weight` column."""
...
def _weighted_mean(per_sequence_loss: torch.Tensor, sample_weights: torch.Tensor) -> torch.Tensor:
    return (per_sequence_loss * sample_weights).sum() / sample_weights.sum().clamp(min=1e-8)


def _sync_cuda_after_loss(model: torch.nn.Module, activation_offloading: bool, device: torch.device) -> None:
    """Finish metric gathers before backward; avoids WSL races with activation offloading streams."""
    if model.training and activation_offloading and device.type == "cuda" and torch.cuda.is_available():
        torch.cuda.synchronize(device)


class DDPSafeDPOTrainer(DPOTrainer):
    """DPO trainer with aligned string tokenization, JSONL sample weights, and DDP-safe ref cache."""

    def __init__(self, *args, use_sample_weight: bool = False, **kwargs):
        self.use_sample_weight = use_sample_weight
        super().__init__(*args, **kwargs)

    def _set_signature_columns_if_needed(self):
        super()._set_signature_columns_if_needed()
        if self.use_sample_weight and self._signature_columns is not None:
            if "weight" not in self._signature_columns:
        ...
        with PartialState().main_process_first():
            ...
            def tokenize_fn(example, processing_class):
                ...
                if "weight" in example:
                    row["weight"] = float(example["weight"])
                return row
    ...
    def _compute_loss(self, model, inputs, return_outputs=False):
        if (
            not model.training
            or not self.use_sample_weight
            or "weight" not in inputs
        ):
            result = super()._compute_loss(model, inputs, return_outputs)
            _sync_cuda_after_loss(model, self.args.activation_offloading, self.accelerator.device)
            return result
        ...
        device = self.accelerator.device
        non_model_keys = {"completion_mask", "ref_chosen_logps", "ref_rejected_logps", "weight"}
        ...
        elif "ref_chosen_logps" in inputs and "ref_rejected_logps" in inputs:
            ref_chosen_logps, ref_rejected_logps = inputs["ref_chosen_logps"], inputs["ref_rejected_logps"]
        else:
            raise RuntimeError("use_sample_weight requires precomputed ref_chosen_logps/ref_rejected_logps columns")
        ...
        for loss_type, loss_weight in zip(self.loss_types, self.loss_weights, strict=True):
            if loss_type == "sigmoid_norm":
                ...
                raise NotImplementedError(
                    f"use_sample_weight only supports sigmoid_norm, got {self.loss_types!r}"
                )
            ...
            if self.use_weighting:
                ...
                wpo_weights = torch.exp(mean_logps)
                chosen_wpo, rejected_wpo = wpo_weights.chunk(2, dim=0)
                per_sequence_loss = per_sequence_loss * chosen_wpo * rejected_wpo

            batch_loss = _weighted_mean(per_sequence_loss, sample_weights) * loss_weight
            loss = batch_loss if loss is None else loss + batch_loss

        with torch.no_grad():
            self._metrics[mode]["dpo/delta"].append(delta.mean().item())
            ...
            chosen_logits, rejected_logits = shift_logits.chunk(2, dim=0)
        ...
        _sync_cuda_after_loss(model, self.args.activation_offloading, device)
...
def _resolve_activation_offloading(requested: bool) -> bool:
    env = os.environ.get("DPO_ACTIVATION_OFFLOAD", "").lower()
    if env in {"0", "false", "no"}:
        return False
    if env in {"1", "true", "yes"}:
        if _is_wsl():
            _log(
                "DPO_ACTIVATION_OFFLOAD=1 on WSL — may crash during backward "
                "('CUDA error: device not ready')"
            )
        return True
    if _is_wsl() and requested:
        _log(
            "Activation offloading disabled on WSL (set DPO_ACTIVATION_OFFLOAD=1 to force; "
            "unstable on multi-GPU backward)"
        )
        return False
    return requested
...
def _resolve_use_weighting(requested: bool, use_sample_weight: bool) -> bool:
    # WPO logsumexp over vocab is heavy on WSL; JSONL sample weights are a separate mechanism.
    if use_sample_weight and os.environ.get("DPO_USE_WEIGHTING", "0").lower() not in {
        "1",
        "true",
        "yes",
    }:
        if requested:
            _log("WPO use_weighting disabled because use_sample_weight=True (set DPO_USE_WEIGHTING=1 for both)")
        return False
...
def dpo_train():
    hp = HYPERPARAMS
    beta = _resolve_float_env("DPO_BETA", hp["beta"])
    learning_rate = _resolve_float_env("DPO_LR", hp["learning_rate"])
    max_length = hp["max_length"]
    ...
    use_weighting = _resolve_use_weighting(hp.get("use_weighting", False), hp.get("use_sample_weight", False))
    use_sample_weight = hp.get("use_sample_weight", False)
    activation_offloading = _resolve_activation_offloading(hp.get("activation_offloading", False))
    ...
    _log(f"JSONL sample weight: {use_sample_weight}")
    _log(f"Activation offloading: {activation_offloading}")
    ...
    trainer = trainer_cls(
        ...
        data_collator=SampleWeightPreferenceCollator(
