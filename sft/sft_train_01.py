...
TEXT_CACHE_DIR = Path("./cache/sft_text_cache")
TEXT_CACHE_META = TEXT_CACHE_DIR / "cache_meta.json"
LEGACY_TOKENIZED_DIR = Path("./cache/sft_tokenized")
...
def load_data(path: Path, tokenizer, max_length: int) -> Dataset:
    ...
    for record in records:
        ...
        truncated_text = tokenizer.decode(ids, skip_special_tokens=False)
        max_seen = max(max_seen, length)
        if length > max_length:
            truncated_length += 1
            # continue
        kept.append({"text": truncated_text})

    print(
        f"Dataset: {len(kept)} kept, {dropped_format} bad format, "
        f"{truncated_length} over max_length ({max_length}), longest={max_seen} tokens"
    )
