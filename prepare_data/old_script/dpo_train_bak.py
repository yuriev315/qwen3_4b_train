...
DPO_CACHE_DIR = Path("data/dpo_dataset_cache")
DPO_CACHE_META = DPO_CACHE_DIR / "cache_meta.json"
LEGACY_DPO_CACHE = Path("data/dpo_data.jsonl.cache")
OUTPUT_DIR = Path("ckpt_dpo")
...
def _remove_legacy_cache() -> None:
    if LEGACY_DPO_CACHE.exists():
        print(f"Removing legacy DPO cache: {LEGACY_DPO_CACHE}")
        shutil.rmtree(LEGACY_DPO_CACHE)


_active_sample_weights: torch.Tensor | None = None
_orig_tensor_mean = torch.Tensor.mean


def _weighted_tensor_mean(self, dim=None, keepdim=False, *, dtype=None):
    if (
        _active_sample_weights is not None
        and dim is None
        and not keepdim
        and self.dim() == 1
        and self.shape[0] == _active_sample_weights.shape[0]
    ):
        weights = _active_sample_weights.to(device=self.device, dtype=self.dtype)
        return (self * weights).sum() / weights.sum().clamp(min=1e-8)
    return _orig_tensor_mean(self, dim=dim, keepdim=keepdim, dtype=dtype)


@contextmanager
def _weighted_loss_context(weights: torch.Tensor):
    global _active_sample_weights
    _active_sample_weights = weights
    torch.Tensor.mean = _weighted_tensor_mean
    try:
        yield
    finally:
        _active_sample_weights = None
        torch.Tensor.mean = _orig_tensor_mean


class WeightedDPOTrainer(DPOTrainer):
    """DPO trainer that applies per-sample weights with a single forward pass per batch."""

    def _compute_loss(self, model, inputs, return_outputs=False):
        weights = inputs.pop("weight", None) if isinstance(inputs, dict) else None
        if weights is None:
            return super()._compute_loss(model, inputs, return_outputs)

        with _weighted_loss_context(weights.to(self.accelerator.device)):
            return super()._compute_loss(model, inputs, return_outputs)
...
def _side_token_lengths(tokenizer, prompt: str, chosen: str, rejected: str) -> tuple[int, int]:
    ...
    rejected_len = len(tokenizer(prompt + _completion_with_eos(tokenizer, rejected), add_special_tokens=False)["input_ids"])
